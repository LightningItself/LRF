...
for j = hcombine : T-hcombine      % Clearly no of output frames = T - 2*hcombine
    ...
    for n = j+1 : hcombine+j % create an output image from 'hcombine' number of images starting from jth image 
        ...
        avg_now=double(hfuse-hfuse); % creting a ZERO 2d matrix of size hfuse
     
        
        for r = n+1-hcombine : 1 : n % Calculating avg of last hcombine images (Last corresponding to nth index)
        ...
        hdeltemp = hssimap_newim - hssimap_fuse ; 
  
        hdeltemp =(hdeltemp+abs(hdeltemp))/2;  
  
        hdelta = double(hdeltemp);
        hdelta = hdelta/max(hdelta(:));
        ...
        hfuse = uint8((ones(size(newim))-hdelta).*double(hfuse)+hdelta.*double(newim));  
