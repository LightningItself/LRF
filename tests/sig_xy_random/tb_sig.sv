...
module tb_top;
    ...
    SIG_XY #(.PIXELS_PER_BEAT(PIXELS_PER_BEAT), .PIXEL_SIZE(PIXEL_SIZE), .IMAGE_DIM(IMAGE_DIM)) dut (
        ...
        .s_axis_tdata_x(s_if[0].tdata), .s_axis_tvalid_x(s_if[0].tvalid), .s_axis_tready_x(s_if[0].tready), .s_axis_tlast_x(s_if[0].tlast),
        .s_axis_tdata_y(s_if[1].tdata), .s_axis_tvalid_y(s_if[1].tvalid), .s_axis_tready_y(s_if[1].tready), .s_axis_tlast_y(s_if[1].tlast),
