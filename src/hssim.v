...
generate
    for (m = 0; m < PIXELS_PER_BEAT; m = m+1) begin : stage4_adders
        ...
        wire signed [2*PIXEL_SIZE+1:0] raw_sum_x_full;
        assign stage4_sign_x[m] = raw_sum_x_full[2*PIXEL_SIZE+1];

        axis_adder #(.DATA_WIDTH(2*PIXEL_SIZE)) numr_part_2_x_adder (
            .aclk(aclk),
            .aresetn(aresetn),
        ...
        assign numr_part_2_x[m*(2*PIXEL_SIZE+2)+:(2*PIXEL_SIZE+2)] = raw_sum_x_full[2*PIXEL_SIZE+1] ? (-raw_sum_x_full) : raw_sum_x_full;
