...
// below we intentionally gave the delay by adding extra reg so that total delay before reaching the mult is 5 cycles( 4 from pipeline and 1 already there in mean calc)
...
// mean terms are delayed a total of 5 cycles
// variance until is 4 cycles dealyed and below we use one reg, so a total of 5 cycles which sync with mean terms
// so before the final numr and denr gets calculate, both mean terms and variance terms are equally delayed by 5 cycles
...
for(j=0; j<PIXELS_PER_BEAT; j=j+1) begin
    always@(posedge clk) begin
        if(~stall) begin
            numr_part_2_x[j*18+:18] <= (out_sig_xy[j*16+:16]<<1) + c2; // one cycle delay for variance terms so that total delay is 5 cycles
