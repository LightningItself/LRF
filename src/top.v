...
always @(posedge s_axis_aclk) begin
    if(!s_axis_aresetn) begin
        ...
        case(state)
            ...
            NEW: begin
                if((first == 1) ? (last_buff_new & avg_lsu_ready & valid_buff_new) : (avg_lsu_ready & sum_last & sum_valid)) begin
                    state <= OLD;
                end
            end
            OLD: begin
...
wire [DATA_WIDTH+(N_FUSE_COUNT+1)*PIXELS_PER_BEAT-1:0] avg_buff_in = ((state == NEW) & (first == 1)) ? iframex17 : int_sum;
wire avg_buff_in_valid = ((state == NEW) & (first == 1)) ? valid_buff_new : sum_valid;
wire avg_buff_in_last = ((state == NEW) & (first == 1)) ? last_buff_new : sum_last;

wire avg_lsu_ready;
wire [DATA_WIDTH+(N_FUSE_COUNT+1)*PIXELS_PER_BEAT-1:0] avg_buff_out;
wire avg_buff_out_valid, avg_buff_out_last;
wire avg_lsu_slave_ready = ((state == NEW) & (first == 1)) ? 0 : (add_ready_x & add_ready_y);
...
// first is only used for seeding the avg lsu with iframex17, low it when avg lsu correctly latches last beat.
always @(posedge s_axis_aclk) begin
    if(!s_axis_aresetn) begin
        first <= 1;
    end
    else if(last_buff_new & avg_lsu_ready & valid_buff_new) begin 
        first <= 0;
    end
end
...
always @(posedge s_axis_aclk) begin
    if(!s_axis_aresetn) begin
        ...
        if((state == NEW) & last_buff_new & add_ready_x & add_ready_y & valid_buff_new) begin
            new_gate <= 0; // so that new_gate wont allow the beat after the last beat into adder. 
        end
...
always @(posedge s_axis_aclk) begin
    if(!s_axis_aresetn) begin
        ...
        if(((state == NEW) & (first == 1)) ? (avg_lsu_ready & s_axis_tvalid) : (avg_lsu_slave_ready & s_axis_tvalid)) begin // avg_lsu_slave_ready also accounts for adders
            data_buff_new <= s_axis_tdata;
            valid_buff_new <= s_axis_tvalid;
            last_buff_new <= s_axis_tlast;
        end
        else if((add_ready_x & add_ready_y & valid_buff_new) || (state == OLD)) begin
            valid_buff_new <= 1'b0;
            last_buff_new <= 1'b0;
        end
    end
