...
wire [((2*(2*(2*PIXEL_SIZE+2))+1)*PIXELS_PER_BEAT)-1:0] old_avg_skid_tdata;
wire old_avg_skid_tvalid, old_avg_skid_tlast, old_avg_skid_tready;

wire [((2*(2*(2*PIXEL_SIZE+2))+1)*PIXELS_PER_BEAT)-1:0] avg_new_skid_tdata;
wire avg_new_skid_tvalid, avg_new_skid_tlast, avg_new_skid_tready;

wire [(2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT-1:0] numr_x, denr_x, numr_z, denr_z;
wire [PIXELS_PER_BEAT-1:0] numr_x_sign, numr_z_sign;

assign numr_x_sign = old_avg_skid_tdata[((2*(2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT) + PIXELS_PER_BEAT)- 1:(2*(2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT)];
assign numr_x = old_avg_skid_tdata[(2*(2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT)-1:(2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT];
assign denr_x = old_avg_skid_tdata[((2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT)-1:0];

assign numr_z_sign = avg_new_skid_tdata[((2*(2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT) + PIXELS_PER_BEAT)- 1:(2*(2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT)];
assign numr_z = avg_new_skid_tdata[(2*(2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT)-1:(2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT];
assign denr_z = avg_new_skid_tdata[((2*(2*PIXEL_SIZE+2))*PIXELS_PER_BEAT)-1:0];
...
wire old_avg_mult_ready = p1_mult_ready_x & p2_mult_ready_y;
wire avg_new_mult_ready = p1_mult_ready_y & p2_mult_ready_x;
...
HSSIM #( .PIXELS_PER_BEAT(PIXELS_PER_BEAT), .PIXEL_SIZE(PIXEL_SIZE), .IMAGE_DIM(IMAGE_DIM)) avg_new (
    ...
    .out_ready(avg_new_skid_tready),
    .out_last(hssim_out_avg_new_last)
);

axis_skid_buff #( .DATA_WIDTH(((2*(2*(2*PIXEL_SIZE+2))+1)*PIXELS_PER_BEAT))) old_avg_skid (
    .aclk(aclk),
    .aresetn(aresetn),
    .s_axis_tdata(hssim_out_old_avg),
    .s_axis_tvalid(hssim_out_old_avg_valid),
    .s_axis_tready(old_avg_skid_tready),
    .s_axis_tlast(hssim_out_old_avg_last),
    .m_axis_tdata(old_avg_skid_tdata),
    .m_axis_tvalid(old_avg_skid_tvalid),
    .m_axis_tready(old_avg_mult_ready),
    .m_axis_tlast(old_avg_skid_tlast)
);

axis_skid_buff #( .DATA_WIDTH(((2*(2*(2*PIXEL_SIZE+2))+1)*PIXELS_PER_BEAT))) avg_new_skid (
    .aclk(aclk),
    .aresetn(aresetn),
    .s_axis_tdata(hssim_out_avg_new),
    .s_axis_tvalid(hssim_out_avg_new_valid),
    .s_axis_tready(avg_new_skid_tready),
    .s_axis_tlast(hssim_out_avg_new_last),
    .m_axis_tdata(avg_new_skid_tdata),
    .m_axis_tvalid(avg_new_skid_tvalid),
    .m_axis_tready(avg_new_mult_ready),
    .m_axis_tlast(avg_new_skid_tlast)
);
...
generate
    for (i = 0; i < PIXELS_PER_BEAT; i = i + 1) begin
        MULTIPLIER #( .DATA_WIDTH((2*(2*PIXEL_SIZE+2))), .mode(0)) p1_mul (
            ...
            .s_axis_tvalid_x(old_avg_skid_tvalid),
            .s_axis_tready_x(p1_mult_ready_1[i]),
            .s_axis_tlast_x(old_avg_skid_tlast),
            .s_axis_tdata_y(denr_z[i*2*(2*PIXEL_SIZE+2)+:2*(2*PIXEL_SIZE+2)]),
            .s_axis_tvalid_y(avg_new_skid_tvalid),
            .s_axis_tready_y(p1_mult_ready_2[i]),
            .s_axis_tlast_y(avg_new_skid_tlast),
        ...
        MULTIPLIER #( .DATA_WIDTH((2*(2*PIXEL_SIZE+2))), .mode(0)) p2_mul (
            ...
            .s_axis_tvalid_x(avg_new_skid_tvalid),
            .s_axis_tready_x(p2_mult_ready_1[i]),
            .s_axis_tlast_x(avg_new_skid_tlast),
            .s_axis_tdata_y(denr_x[i*2*(2*PIXEL_SIZE+2)+:2*(2*PIXEL_SIZE+2)]),
            .s_axis_tvalid_y(old_avg_skid_tvalid),
            .s_axis_tready_y(p2_mult_ready_2[i]),
            .s_axis_tlast_y(old_avg_skid_tlast),
...
always @(posedge aclk) begin
    if(!aresetn) begin
        ...
        if(old_avg_mult_ready & old_avg_skid_tvalid) begin
            numr_x_sign_1 <= numr_x_sign;
        end
        if(avg_new_mult_ready & avg_new_skid_tvalid) begin
