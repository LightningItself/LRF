...
wire [DATA_WIDTH-1:0] old_edge_skid, avg_edge_skid, new_edge_skid;
wire old_edge_skid_valid, old_edge_skid_last, avg_edge_skid_valid, avg_edge_skid_last, new_edge_skid_valid, new_edge_skid_last;
wire old_edge_skid_ready, avg_edge_skid_ready, new_edge_skid_ready;
...
CONV_SOBEL #( .PIXELS_PER_BEAT(PIXELS_PER_BEAT), .PIXEL_SIZE(PIXEL_SIZE), .IMAGE_WIDTH(IMAGE_DIM), .DATA_WIDTH(DATA_WIDTH)) sob_old(
    ...
    .m_axis_tready(old_edge_skid_ready),
...
CONV_SOBEL #( .PIXELS_PER_BEAT(PIXELS_PER_BEAT), .PIXEL_SIZE(PIXEL_SIZE), .IMAGE_WIDTH(IMAGE_DIM), .DATA_WIDTH(DATA_WIDTH)) sob_avg(
    ...
    .m_axis_tready(avg_edge_skid_ready),
...
CONV_SOBEL #( .PIXELS_PER_BEAT(PIXELS_PER_BEAT), .PIXEL_SIZE(PIXEL_SIZE), .IMAGE_WIDTH(IMAGE_DIM), .DATA_WIDTH(DATA_WIDTH)) sob_new(
    ...
    .m_axis_tready(new_edge_skid_ready),
    .m_axis_tlast(new_edge_last)
);

axis_skid_buff #( .DATA_WIDTH(DATA_WIDTH)) old_edge_skid_buff(
    .aclk(aclk),
    .aresetn(aresetn),
    .s_axis_tdata(old_edge),
    .s_axis_tvalid(old_edge_valid),
    .s_axis_tready(old_edge_skid_ready),
    .s_axis_tlast(old_edge_last),
    .m_axis_tdata(old_edge_skid),
    .m_axis_tvalid(old_edge_skid_valid),
    .m_axis_tready(hssim_ready_x),
    .m_axis_tlast(old_edge_skid_last)
);

axis_skid_buff #( .DATA_WIDTH(DATA_WIDTH)) avg_edge_skid_buff(
    .aclk(aclk),
    .aresetn(aresetn),
    .s_axis_tdata(avg_edge),
    .s_axis_tvalid(avg_edge_valid),
    .s_axis_tready(avg_edge_skid_ready),
    .s_axis_tlast(avg_edge_last),
    .m_axis_tdata(avg_edge_skid),
    .m_axis_tvalid(avg_edge_skid_valid),
    .m_axis_tready(hssim_ready_y),
    .m_axis_tlast(avg_edge_skid_last)
);

axis_skid_buff #( .DATA_WIDTH(DATA_WIDTH)) new_edge_skid_buff(
    .aclk(aclk),
    .aresetn(aresetn),
    .s_axis_tdata(new_edge),
    .s_axis_tvalid(new_edge_valid),
    .s_axis_tready(new_edge_skid_ready),
    .s_axis_tlast(new_edge_last),
    .m_axis_tdata(new_edge_skid),
    .m_axis_tvalid(new_edge_skid_valid),
    .m_axis_tready(hssim_ready_z),
    .m_axis_tlast(new_edge_skid_last)
);

HSSIM_TOP #( .PIXELS_PER_BEAT(PIXELS_PER_BEAT), .PIXEL_SIZE(PIXEL_SIZE), .IMAGE_DIM(IMAGE_DIM)) hssim(
    .aclk(aclk),
    .aresetn(aresetn),
    .old_map(old_edge_skid),
    .old_map_valid(old_edge_skid_valid),
    .old_map_ready(hssim_ready_x),
    .old_map_last(old_edge_skid_last),
    .avg_map(avg_edge_skid),
    .avg_map_valid(avg_edge_skid_valid),
    .avg_map_ready(hssim_ready_y),
    .avg_map_last(avg_edge_skid_last),
    .new_map(new_edge_skid),
    .new_map_valid(new_edge_skid_valid),
    .new_map_ready(hssim_ready_z),
    .new_map_last(new_edge_skid_last),
    .del(del_int),
...
assign old_map_ready = sob_old_ready & avg_map_valid & new_map_valid;
assign avg_map_ready = sob_avg_ready & old_map_valid & new_map_valid;
assign new_map_ready = sob_new_ready & old_map_valid & avg_map_valid;
